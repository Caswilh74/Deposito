...
/*   Updated: 2024/10/24 15:51:29 by gcarvalh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char    *ft_strchr(const char *s, int c);  // Finds the first occurrence of a character
char    *ft_strrchr(const char *s, int c); // Finds the last occurrence of a character
int     ft_strncmp(const char *s1, const char *s2, size_t n);  // Compares two strings lexicographically
size_t  ft_strlcpy(char *dst, const char *src, size_t dstsize);  // Copies a string to a destination buffer
size_t  ft_strlcat(char *dst, const char *src, size_t dstsize);  // Concatenates two strings
char    *ft_strnstr(const char *haystack, const char *needle, size_t len);  // Finds a substring within a string
char    *ft_strdup(const char *s);  // Duplicates a string
char    *ft_substr(char const *s, unsigned int start, size_t len);  // Extracts a substring
char    *ft_strjoin(char const *s1, char const *s2);  // Concatenates two strings
char    *ft_strtrim(char const *s1, char const *set);  // Trims characters from a string
char    **ft_split(char const *s, char c);  // Splits a string into an array of strings
...
void    *ft_memset(void *b, int c, size_t len);  // Fills a block of memory with a given value
void    *ft_memcpy(void *dst, const void *src, size_t n);  // Copies memory from one location to another
void    *ft_memmove(void *dst, const void *src, size_t n);  // Copies memory, handling overlapping regions
void    *ft_memchr(const void *s, int c, size_t n);  // Finds the first occurrence of a character in a memory block
